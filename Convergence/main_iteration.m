...
R3MC_CG=true; R3MC_TR=true; 
EmbG_CG=true; LMaFit=true;
savedata=false;
%% Problem data
...
semilogy([1:length(NR1)], sqrt(NR1.*(2/K)), 'r-','LineWidth',1.2, 'MarkerSize',6);
hold on;
end

if R3MC_TR==true
NR2=[info_R3MC_TR.cost]; Time2=[info_R3MC_TR.time]; Time2=Time2(length(Time2));
semilogy([1:length(NR2)], sqrt(NR2.*(2/K)), 'b-','LineWidth',1.2, 'MarkerSize',6);
hold on;
end
...
semilogy([1:length(NR3)], sqrt(NR3.*(2/K)), 'g-','LineWidth',1.2, 'MarkerSize',6);
hold on;
end
...
if LMaFit==true
NR4=Out_LMaFit.obj;
semilogy([1:length(NR4)], NR4, 'm-','LineWidth',1.2, 'MarkerSize',6);
hold on;
end
